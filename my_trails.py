    ...
    dbc.Row([
        dbc.Col(
            html.Header([
                ...
                html.Ul([
                    html.Li(dcc.Link('Home', href='/', className='link')),
...
def display_image_marker(contents, trail, status, zoom):
    markers = []
    if not trail:
    ...
    df_uploads = pd.DataFrame(columns=['upload_id', 'upload_lat', 'upload_long', 'upload_time', 'upload_img', 'upload_species'])
